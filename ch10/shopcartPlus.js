...
$(function(){
  ...
  var $totalPriceInput= $('.totalPrice input[type=checkbox]');//计算总价中的全选
  ...
  $theadInput.change(function(){
    ...
    $totalPriceInput.prop('checked',checkState);//把状态给计算总价中的全选状态
 ...
 $totalPriceInput.change(function(){
   var checkState = $(this).prop('checked');//获取计算总价中全选状态
...
$tbodyInputs.change(function(){//给表格中单选框绑定事件
  ...
  $tbodyInputs.each(function(index,input){//循环选中状态
...
/*
    减法功能
       1.给 - 绑定点击事件
       2.获取前面输入框的值
       3.值自减 边界判断 如果校园1 那么等于1 否则等于自己
       4.把减少后的值 重新赋值给后面的输入框
*/
...
$('.reduce').click(function(){//绑定事件
  ...
  $(this).prev().val(count);//.把减少后的值 重新赋值给后面的输入框
...
/*
 ...
 //找到单价
  ...
  dom.closest('tr').find('.subprice').text(subTotalPrice.toFixed(2)) //把小计结果渲染到对应位置 保留两位小数
